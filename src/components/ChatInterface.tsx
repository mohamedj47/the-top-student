  ...
  const handleSend = async (text: string = inputValue, specialPrompt?: string) => {
    ...
    try {
      ...
      await generateStreamResponse(
        ...
        userMessage.attachment, undefined, localStorage.getItem('device_id') || 'local'
      );
      
      setMessages(prev => prev.map(msg => msg.id === botId ? { ...msg, isStreaming: false } : msg));
    } catch (err) { 
      setMessages(prev => [...prev, { id: 'err', text: "عذراً، واجهت مشكلة في الاتصال.", sender: Sender.BOT, timestamp: new Date() }]);
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 chat-container">
      <Suspense fallback={null}>
        <LessonBrowser isOpen={isLessonBrowserOpen} onClose={() => setIsLessonBrowserOpen(false)} grade={grade} subject={subject} onPlayVideo={(l, d) => { setCurrentLessonTitle(l); setCurrentVideoData(d); setIsVideoModalOpen(true); }} onExplain={(l) => { setIsLessonBrowserOpen(false); handleSend(`اشرح درس "${l}" بالتفصيل`); }} />
      ...
      <div className="p-4 bg-white border-t border-slate-200 space-y-4 shadow-[0_-10px_25px_rgba(0,0,0,0.03)] no-print">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 relative flex items-center">
            <textarea ref={textareaRef} value={inputValue} onChange={e => setInputValue(e.target.value)} onKeyDown={e => e.key === 'Enter' && !e.shiftKey && (e.preventDefault(), handleSend())} placeholder="اكتب سؤالك هنا..." className="w-full bg-slate-50 border-none rounded-2xl px-4 py-4 h-[58px] focus:ring-2 focus:ring-indigo-100 outline-none font-bold text-sm shadow-inner" />
            <button onClick={() => handleSend()} disabled={(!inputValue.trim() && !selectedAttachment) || isLoading} className={`absolute left-2.5 p-2.5 rounded-xl transition-all ${inputValue.trim() || selectedAttachment ? 'bg-indigo-600 text-white' : 'bg-slate-200 text-slate-400 cursor-not-allowed'}`}><Send size={20} className="rotate-[-180deg]" /></button>
          </div>
        </div>
      </div>
    </div>
