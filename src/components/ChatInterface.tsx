...
import { 
  Send, ChevronRight, List, Bot, Loader2, Target, Mic, Camera, X, Star, ShieldCheck, PenTool, LayoutGrid, Database, Brain, BookOpen, Search, Sparkles
} from 'lucide-react';
...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({ grade, subject, studyLanguage = StudyLanguage.ARABIC, onBack, onSubscribe }) => {
  ...
  useEffect(() => {
    const historyKey = `history_v3_${grade}_${subject}`;
  ...
  useEffect(() => {
    localStorage.setItem(`history_v3_${grade}_${subject}`, JSON.stringify(messages));
  ...
  const handleSend = async (text: string = inputValue) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { id: 'err', text: "المحرك في وضع الصيانة التلقائية (Deca-Shield)، جاري استعادة الاتصال...", sender: Sender.BOT, timestamp: new Date() }]);
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans" dir="rtl">
        ...
        <div className="p-6 border-b border-slate-100 bg-indigo-600 text-white">
          <div className="flex items-center gap-3">
             ...
             <div>
                ...
                <p className="text-[10px] text-indigo-100 font-black uppercase">Source Studio v4.0</p>
             </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
           <div className="bg-indigo-50 p-4 rounded-3xl border border-indigo-100">
              ...
              <p className="text-[10px] text-slate-500 font-bold leading-relaxed">
                يستخدم النظام الآن 11 مفتاح API بالتناوب لضمان استقرار الخدمة.
              </p>
           </div>

           <button onClick={() => setIsLessonBrowserOpen(true)} className="w-full p-4 bg-white hover:bg-slate-50 rounded-2xl border border-slate-200 shadow-sm transition-all text-right group flex items-center gap-3">
           ...
           <button onClick={() => setIsNotebookOpen(true)} className="w-full p-4 bg-white hover:bg-slate-50 rounded-2xl border border-slate-200 shadow-sm transition-all text-right group flex items-center gap-3">
      ...
      <main className="flex-1 flex flex-col relative bg-white">
        ...
        <header className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-white/80 backdrop-blur-md sticky top-0 z-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] text-indigo-600 font-black">نظام المعلم الخارق • أوفلاين نشط ✅</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-10 space-y-6 scrollbar-hide">
          <div className="max-w-4xl mx-auto w-full chat-container">
            {messages.map(m => <MessageBubble key={m.id} message={m} subject={subject} />)}
      ...
      {/* Modals */}
      <LessonBrowser isOpen={isLessonBrowserOpen} onClose={() => setIsLessonBrowserOpen(false)} grade={grade} subject={subject} onExplain={(l) => { setIsLessonBrowserOpen(false); handleSend(`اشرح لي درس ${l} بالتفصيل بلهجة مصرية.`); }} onPlayVideo={() => {}} />
