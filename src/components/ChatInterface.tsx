...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({ grade, subject, studyLanguage = StudyLanguage.ARABIC, onBack, onSubscribe }) => {
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 chat-container" dir={studyLanguage === StudyLanguage.ARABIC ? 'rtl' : 'ltr'}>
      {/* Modals are now direct for faster response */}
      <LessonBrowser 
        ...
        studyLanguage={studyLanguage} 
        onPlayVideo={(l, d) => { 
          setIsLessonBrowserOpen(false); // Close browser to avoid z-index conflicts
        ...
        onExplain={(l) => { 
          ...
          handleSend(studyLanguage === StudyLanguage.ARABIC ? `اشرح درس "${l}" بالتفصيل` : `Explain lesson "${l}" in detail`); 
        }} 
      />
