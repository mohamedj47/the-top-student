    ...
    <div className="flex flex-col h-screen bg-slate-50 chat-container" dir={studyLanguage === StudyLanguage.ARABIC ? 'rtl' : 'ltr'}>
        ...
        <div className={`flex items-center gap-2 flex-1 ${studyLanguage === StudyLanguage.ARABIC ? 'flex-row' : 'flex-row-reverse'}`}>
          ...
          <div className={studyLanguage === StudyLanguage.ARABIC ? 'text-right' : 'text-left'}>
            ...
            <div className="flex items-center gap-2 relative">
                ...
                {cloudConnected ? (
                    <span className="flex items-center gap-1.5 text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-full border border-emerald-100 shadow-sm transition-all hover:bg-emerald-100 cursor-pointer">
      ...
      <div className="p-4 bg-white border-t border-slate-200 space-y-4 shadow-[0_-10px_25px_rgba(0,0,0,0.03)] no-print">
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
          <QuickTool icon={Brain} label={studyLanguage === StudyLanguage.ARABIC ? "أسئلة الدرس المتوقعة" : "Lesson Forecast Qs"} color="text-violet-600 border-violet-100 bg-violet-50/30" prompt={studyLanguage === StudyLanguage.ARABIC ? `بناءً على الدرس الذي شرحته لي للتو أو ناقشناه، أريد "الأسئلة النهائية" التي تضمن لي فهم الدرس بنسبة 100%. وفر لي أهم وأصعب التركات المتوقعة في الامتحان لهذا الدرس تحديداً مع الإجابات النموذجية.` : `Based on the lesson we just discussed, generate the "Ultimate Questions" that guarantee 100% mastery. Provide the most expected and challenging exam-style questions for this specific lesson with ideal answers.`} />
          <QuickTool icon={Sparkles} label={studyLanguage === StudyLanguage.ARABIC ? "عصارة ليلة الامتحان" : "Final Night Booklet"} color="text-amber-600 border-amber-100 bg-amber-50/30" prompt={studyLanguage === StudyLanguage.ARABIC ? `يا دكتور، أعطني "المذكرة الذهبية" لليلة الامتحان في مادة ${subject}. ركز على أهم التركات والمتوقع.` : `Doctor, give me the "Golden Night Booklet" for ${subject}. Focus on the most expected points.`} />
        </div>
